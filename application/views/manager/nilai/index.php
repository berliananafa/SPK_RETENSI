<!-- Nilai Sub Kriteria CS - Junior Manager (Read-Only) -->
<div class="row">
	<div class="col-12">
		<div class="card shadow">
			<div class="card-header bg-gradient-primary text-white">
				<strong class="card-title mb-0">Monitoring Data Penilaian Customer Service</strong>
				<span class="small ml-2" style="opacity: 0.9;">Monitoring nilai penilaian CS di tim Anda (Read-Only)</span>
			</div>
			<div class="card-body">
				<!-- Filter Section -->
				<div class="row mb-3">
							...
							<tbody>
								<?php foreach ($nilai_data as $index => $nilai): ?>
									<tr>
										...
										<td>
											<small class="text-muted"><?= htmlspecialchars($nilai->nama_tim ?? '-') ?></small>
										</td>
										<td>
											<small class="text-muted"><?= htmlspecialchars($nilai->nama_supervisor ?? '-') ?></small>
										</td>
										<td>
...
<script>
	$(document).ready(function() {
		...
		$('#btnFilter').on('click', function() {
			...
			var url = '<?= base_url('junior-manager/nilai') ?>?';
			var params = [];
