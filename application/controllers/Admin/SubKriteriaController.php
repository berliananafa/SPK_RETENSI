...
class SubKriteriaController extends Admin_Controller
{
	...
	public function index()
	{
		...
		$data['total_kriteria'] = count($data['sub_kriteria']);
		$data['total_pending'] = 0;
		$data['total_approved'] = 0;
		$data['total_rejected'] = 0;

		foreach ($data['sub_kriteria'] as $sk) {
			$status = $sk->sub_kriteria_status_approval ?? 'pending';
			if ($status === 'pending') {
				$data['total_pending']++;
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
            ...
            $hasChanges = (
                $sub_kriteria->id_kriteria !== $this->input->post('id_kriteria', true) ||
                $sub_kriteria->nama_sub_kriteria !== $this->input->post('nama_sub_kriteria', true) ||
                $sub_kriteria->bobot_sub !== $this->input->post('bobot_sub', true) 
            );
			// Data update
			$data = [
				'id_kriteria'        => $this->input->post('id_kriteria', true),
