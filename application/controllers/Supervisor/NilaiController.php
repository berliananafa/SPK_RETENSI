...
class NilaiController extends Supervisor_Controller
{
    ...
    /**
     * Tampilkan nilai sub kriteria CS (read-only untuk supervisor)
     */
    public function index()
    {
        set_page_title('Nilai Sub Kriteria - Supervisor');
        set_breadcrumb([
        ...
        // Get filter options
        $data['periodes'] = $this->NilaiModel->getDistinctPeriodesBySupervisor($supervisorId);
        $data['teams'] = $this->TimModel->getBySupervisorWithDetails($supervisorId);
        $data['cs_list'] = $this->CustomerServiceModel->getBySupervisor($supervisorId);
        ...
        $data['nilai_data'] = $this->NilaiModel->getNilaiWithDetailsBySupervisor($supervisorId, $filter);
        ...
        render_layout('supervisor/nilai/index', $data);
    }
}
