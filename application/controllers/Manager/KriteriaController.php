...
class KriteriaController extends Manager_Controller
{
	...
	/**
	 * Menyetujui sub kriteria (Approve)
	 * Request hanya boleh melalui AJAX dan metode POST
	 */
	public function approve_sub($id)
	{
		// Validasi request AJAX dan metode POST
		if (!$this->input->is_ajax_request() || $this->input->method() !== 'post') {
			show_error('Invalid request method', 405);
		...
		// Cari data sub kriteria berdasarkan ID
		...
		// Proses approve menggunakan method di model
		if ($this->SubKriteriaModel->approveById($id, $userId)) {
			$this->output->set_content_type('application/json');
			echo json_encode([
				'status'  => 'success',
				'message' => 'Sub Kriteria berhasil disetujui'
			]);
		} else {
			$this->output->set_status_header(500);
			echo json_encode([
	...
	/**
	 * Menolak sub kriteria (Reject)
	 * Request hanya boleh melalui AJAX dan metode POST
	 */
	public function reject_sub($id)
	{
		// Validasi request AJAX dan metode POST
		if (!$this->input->is_ajax_request() || $this->input->method() !== 'post') {
			show_error('Invalid request method', 405);
		...
		// Proses reject menggunakan method di model
		if ($this->SubKriteriaModel->rejectById($id, $userId, $keterangan)) {
			$this->output->set_content_type('application/json');
			echo json_encode([
				'status'  => 'success',
				'message' => 'Sub Kriteria berhasil ditolak'
			]);
		} else {
			$this->output->set_status_header(500);
			echo json_encode([
