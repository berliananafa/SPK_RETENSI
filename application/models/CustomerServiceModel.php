...
class CustomerServiceModel extends MY_Model
{
	...
	/**
	 * Ambil data Customer Service dengan multiple filters
	 *
	 * @param array $filters Array berisi filter ['id_tim' => 1, 'id_produk' => 2, 'id_kanal' => 3]
	 * @return array
	 */
    ...
    ');
		...
		$this->db->join('tim t', 'cs.id_tim = t.id_tim', 'left');
		$this->db->join('produk p', 'cs.id_produk = p.id_produk', 'left');
		$this->db->join('kanal k', 'cs.id_kanal = k.id_kanal', 'left');

		// Apply filters jika ada
		if (!empty($filters)) {
			foreach ($filters as $field => $value) {
				if ($value) {
					$this->db->where('cs.' . $field, $value);
				}
			}
		}

		$this->db->order_by('cs.nama_cs', 'ASC');
	...
	/**
	 * Ambil data Customer Service berdasarkan Produk
	...
	{
		return $this->getWithFilters(['id_produk' => $id_produk]);
	}

	/**
	 * Ambil data Customer Service berdasarkan Tim
	 *
	 * @param int $id_tim ID Tim
	 * @return array
	 */
	public function getByTim($id_tim)
	{
		return $this->getWithFilters(['id_tim' => $id_tim]);
	}

	/**
	 * Ambil data Customer Service berdasarkan Kanal
	 *
	 * @param int $id_kanal ID Kanal
	 * @return array
	 */
	public function getByKanal($id_kanal)
	{
		return $this->getWithFilters(['id_kanal' => $id_kanal]);
	}
